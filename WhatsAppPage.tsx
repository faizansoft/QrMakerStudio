...
const WhatsAppPage: React.FC<PageProps> = (props) => {
  ...
  useEffect(() => {
    document.title = "WhatsApp QR Code Generator | Direct Chat Automation";
    const metaDesc = document.querySelector('meta[name="description"]');
    if (metaDesc) {
      metaDesc.setAttribute('content', 'Generate a professional WhatsApp QR code with a pre-filled message. Let customers start a chat instantly. Free SVG exports for business cards and labels.');
    }
  }, []);
      ...
      <article className="max-w-5xl mx-auto px-6 py-24 space-y-24">
        ...
        <section className="pt-12 border-t border-slate-100 text-center space-y-8">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <Link to="/facebook-qr-code-generator" className="px-6 py-3 bg-white border border-slate-200 rounded-full text-xs font-bold text-slate-600 hover:border-indigo-600 hover:text-indigo-600 transition-all">Facebook Growth</Link>
